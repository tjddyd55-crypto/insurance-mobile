...
function sha256Blocks(bytes: Uint8Array): Uint8Array {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    compressBlock(view, offset, hash, words);
  }
...
export function sha256HexSync(bytes: Uint8Array): string {
  return [...sha256Blocks(bytes)].map((byte) => byte.toString(16).padStart(2, '0')).join('');
}
