    ...
    <View style={styles.root}>
      ...
      <Screen padded={false}>
        <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={query.isRefetching} onRefresh={() => void query.refetch()} colors={[theme.colors.primary]} tintColor={theme.colors.primary} />}>
          ...
          <Inline><Button label="손해보험사" size="sm" variant={category === 'non_life' ? 'selected' : 'secondary'} onPress={() => setCategory('non_life')} style={styles.tab} /><Button label="생명보험사" size="sm" variant={category === 'life' ? 'selected' : 'secondary'} onPress={() => setCategory('life')} style={styles.tab} /></Inline>
          {query.isLoading ? <AppText variant="caption">보험사 목록을 불러오는 중…</AppText> : null}
  ...
  return (
    <Card variant="outlined">
      <Stack gap="md">
        ...
        <Button label="설계사이트 →" fullWidth variant="secondary" disabled={!safeExternalUrl(site.salesUrl)} onPress={() => void open(site.salesUrl)} />
