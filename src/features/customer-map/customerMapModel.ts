import { mapApiGenderLabelToGender } from './customerMapSelectionPresentation';
import type { CustomerMapItem, CustomerMapResult, CustomerMapUnmapped } from './types';

function normalizeMapGender(row: Record<string, unknown>) {
  const rawGender = row.gender;
  if (rawGender === 'male' || rawGender === 'female') {
    return rawGender;
  }
  return mapApiGenderLabelToGender(String(row.genderLabel ?? ''));
}

function itemOf(value: unknown): CustomerMapItem | null {
  if (!value || typeof value !== 'object') return null;
  const row = value as Record<string, unknown>;
  const id = Number(row.id);
  const latitude = Number(row.latitude);
  const longitude = Number(row.longitude);
  if (!Number.isFinite(id) || !Number.isFinite(latitude) || !Number.isFinite(longitude)) return null;
  const birthDateRaw = String(row.birthDateYmd ?? row.birthDate ?? '').trim();
  return {
    id,
    name: String(row.name ?? ''),
    phone: String(row.phone ?? ''),
    address: String(row.address ?? ''),
    birthDateYmd: birthDateRaw || null,
    gender: normalizeMapGender(row),
    latitude,
    longitude,
    lastConsultDate: row.lastConsultDate == null ? null : String(row.lastConsultDate),
    isFavorite: Boolean(row.isFavorite),
    markerNo: Number(row.markerNo) || 0,
  };
}
