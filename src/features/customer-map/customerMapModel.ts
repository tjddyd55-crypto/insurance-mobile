...
function itemOf(value: unknown): CustomerMapItem | null { if (!value || typeof value !== 'object') return null; const row = value as Record<string, unknown>; const id = Number(row.id); const latitude = Number(row.latitude); const longitude = Number(row.longitude); if (!Number.isFinite(id) || !Number.isFinite(latitude) || !Number.isFinite(longitude)) return null; return { id, name: String(row.name ?? ''), phone: String(row.phone ?? ''), address: String(row.address ?? ''), latitude, longitude, lastConsultDate: row.lastConsultDate == null ? null : String(row.lastConsultDate), isFavorite: Boolean(row.isFavorite), markerNo: Number(row.markerNo) || 0 }; }
