...
import {
  createAccountVaultRow,
  deleteAccountVaultRow,
  getAccountVault,
...
const TABS: { id: AccountCategory; label: string }[] = [
  { id: 'LIFE', label: '생명보험' },
  { id: 'NON_LIFE', label: '손해보험' },
  { id: 'GENERAL', label: '일반' },
];
...
export function AccountVaultScreen() {
  ...
  const [category, setCategory] = useState<AccountCategory>('LIFE');
  const [addOpen, setAddOpen] = useState(false);
  const [deleting, setDeleting] = useState<AccountVaultRow | null>(null);
  const [message, setMessage] = useState('');
  const query = useQuery({
    queryKey: QUERY_KEY,
    queryFn: () => getAccountVault(token),
    enabled: Boolean(token),
  });
  const remove = useMutation({
    mutationFn: (id: string) => deleteAccountVaultRow(token, id),
    onSuccess: async () => {
      setDeleting(null);
  ...
  const rows = (query.data ?? [])
    .filter((row) => row.category === category && !row.isArchived)
  ...
  return (
    <View style={styles.root}>
      ...
      <Screen padded={false}>
        ...
        >
          <AppText variant="caption">
            보험사 로그인 정보는 서버에서 조회하며 기기에 별도로 저장하지 않습니다.
          </AppText>
          <Inline>
            {TABS.map((tab) => (
              <Button
      ...
      <ConfirmDialog
        ...
        title="계정 삭제"
        message={`${deleting?.companyName ?? ''} 계정을 삭제하시겠습니까?`}
        confirmLabel="삭제"
        tone="danger"
...
function AccountCard({
  ...
  }, [row.loginId, row.loginPassword]);
  const save = useMutation({
    mutationFn: () =>
      updateAccountVaultRow(token, row.id, { loginId, loginPassword: password }),
    ...
    <Card variant="outlined">
      <Stack gap="md">
        ...
        <Inline>
          ...
          />
          {row.isCustom ? (
            <Button label="삭제" variant="danger" onPress={onDelete} style={styles.tab} />
          ) : null}
        </Inline>
      </Stack>
    </Card>
