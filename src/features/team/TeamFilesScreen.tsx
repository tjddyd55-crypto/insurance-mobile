    ...
    <View style={styles.root}>
      ...
      <Screen padded={false}>
        <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={filesQuery.isRefetching} onRefresh={() => void filesQuery.refetch()} colors={[theme.colors.primary]} tintColor={theme.colors.primary} />}>
          ...
          <Card variant="filled"><AppText variant="bodyStrong">팀 저장공간</AppText><AppText color="textSecondary">{membersQuery.isLoading ? '사용량 불러오는 중…' : membersQuery.data && membersQuery.data.teamStorageLimitBytes > 0 ? `${formatStorageBytes(membersQuery.data.teamStorageUsedBytes)} / ${formatStorageBytes(membersQuery.data.teamStorageLimitBytes)}` : '용량 정보를 표시할 수 없습니다.'}</AppText></Card>
