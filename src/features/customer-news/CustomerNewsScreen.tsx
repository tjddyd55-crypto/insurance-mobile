...
function LinkedCustomerPicker({ rows, selectedId, onSelect }: { rows: LinkedCustomer[]; selectedId: number | null; onSelect: (id: number) => void }) { return <View><AppText variant="label">받을 고객</AppText><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingVertical: 8 }}>{rows.map((item) => <Button key={item.customerId} label={`${item.customerName} · 기기 ${item.deviceCount}`} size="sm" variant={selectedId === item.customerId ? 'selected' : 'secondary'} onPress={() => onSelect(item.customerId)} />)}</ScrollView>{!rows.length ? <AppText variant="caption">고객 앱에 연결된 고객이 없습니다.</AppText> : null}</View>; }
