...
export const COVERAGE_ITEM_CATALOG: CatalogItem[] = [
  ...
  { id: 'cancer-diagnosis', label: '암 진단금', category: 'diagnosis' },
  { id: 'cancer-surgery', label: '암 수술비', category: 'treatment' },
  { id: 'chemo', label: '항암약물치료', category: 'treatment' },
  { id: 'radiation', label: '방사선치료', category: 'treatment' },
];

export function catalogItemsForTab(tab: CatalogTabId): CatalogItem[] {
  if (tab === 'favorite') return [];
  if (tab === 'treatment') {
    return COVERAGE_ITEM_CATALOG.filter((item) => ['diagnosis', 'treatment', 'recovery'].includes(item.category));
  }
  if (tab === 'support') return COVERAGE_ITEM_CATALOG.filter((item) => item.category === 'support');
