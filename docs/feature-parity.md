...
Menu labels/order mirror insurance `buildAppMenuForSession` (USER), except the native-only
`고객 상담` section requested ahead of the web menu.
...
| 고객 상담 | 내 바인더 | /personal-binders | /customer-consulting/personal-binders | CustomerConsultingWebScreen | 웹 develop `/personal-binders`, API `/backend/personal-binders`. 세션은 localStorage 주입. PDF는 공유 시트로 연다 | NATIVE | WEBVIEW_TEMP | DEV 앱만. 스와이프·핀치·PDF 공유. 토큰 URL 금지. 프로덕션 앱·프로덕션 호스트는 열지 않음 |
| 고객 상담 | 시뮬레이션 | /coverage-simulator | /customer-consulting/coverage-simulation | CustomerConsultingWebScreen | 웹 develop 보장 시뮬레이션. 진입은 시나리오 선택(`/coverage-simulator`). 세션은 localStorage 주입 | NATIVE | WEBVIEW_TEMP | DEV 앱만. 토큰 URL 금지. 프로덕션 앱·프로덕션 호스트는 열지 않음 |
